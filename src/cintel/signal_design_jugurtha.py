"""
signal_design_jugurtha.py - Project script (example).
...
Paths (relative to repo root)
    ...
    INPUT FILE: data/system_metrics_jugurtha.csv
    OUTPUT FILE: artifacts/signals_jugurtha.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.signal_design_jugurtha
...
DATA_FILE: Final[Path] = DATA_DIR / "system_metrics_jugurtha.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "signals_jugurtha.csv"
...
def main() -> None:
    ...
    # ----------------------------------------------------
    # STEP 2.6b: DEFINE THE LATENCY SPIKE SIGNAL RECIPE
    # ----------------------------------------------------
    # Flag observations where average latency exceeds 30ms.
    # True means a spike was detected, False means normal.
    LATENCY_SPIKE_THRESHOLD_MS: Final[float] = 30.0

    latency_spike_signal_recipe: pl.Expr = (
        pl.when(is_requests_positive)
        .then(calculated_avg_latency > LATENCY_SPIKE_THRESHOLD_MS)
        .otherwise(False)
        .alias("latency_spike")
    )
    ...
    df_with_signals: pl.DataFrame = df.with_columns(
        [
            ...
            latency_spike_signal_recipe,
        ]
    )

    LOG.info(
        "Created signal columns: error_rate, avg_latency_ms, throughput, latency_spike"
    )
